...
class AdminController extends BaseController
{
	...
	function index()
	{
		...
		else
		{
			$data	= array(
				'page'			=> 'error',
				'sub'			=> 'Not Allowed',
				'data'			=> array('title'=> 'Your Account is Not Active', 'description' => 'You cannot login use this account because your account have deactivated or inactive')
			);
			return View::make('admin.template', $data);
		}
	}
	...
	function post($sub = '', $i = '')
	{
		if(count(PostCategory::getCategory()) > 0)
		{
			...
			elseif($sub == 'reject')
			{
				if(Auth::user()->isAdmin() || Auth::user()->isEditor())
				{
					PostEditor::PostUpdateReject($i);
					return Redirect::to('admin/post/review')->with('message','Draft Rejected');
				}
				else
				{
					return redirect('505');
	...
	function settingAction($sub = '', $i = '', Request $r)
	{
		...
		elseif($sub == 'language')
		{
			SettingBlog::updateDualLanguage($r);
			return Redirect::back()->with('message', 'Configuration Updated !');
		}
	}
